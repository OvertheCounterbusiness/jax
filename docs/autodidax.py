...
def cond(pred, true_fn, false_fn, *operands):
  ...
  if typecheck_jaxpr(true_jaxpr) != typecheck_jaxpr(false_jaxpr):
    raise TypeError
  outs = bind(cond_p, pred, *true_consts, *false_consts, *operands,
...
def cond_partial_eval(trace, tracers, *, true_jaxpr, false_jaxpr):
  ...
  *jaxprs, out_uks, num_res = _cond_partial_eval(true_jaxpr, false_jaxpr, in_uks)
  ...
  outs1, res = split_list(outs1_res, num_res)
  ...
  for t in outs2: t.recipe = eqn
  outs1, outs2 = iter(outs1), iter(outs2)
  return [next(outs2) if uk else next(outs1) for uk in out_uks]
...
def cond_transpose_rule(cts, pred, *invals, true_jaxpr, false_jaxpr):
  undef_primals = [type(x) is UndefPrimal for x in invals]
  true_jaxpr_transposed, true_consts = transpose_jaxpr(true_jaxpr, tuple(undef_primals))
  false_jaxpr_transposed, true_consts = transpose_jaxpr(true_jaxpr, tuple(undef_primals))
  breakpoint()
